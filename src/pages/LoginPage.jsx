...
export function LoginPage() {
  ...
  async function finishLogin(idToken) {
    ...
    notifySuccess(`Welcome back, ${user.displayName}!`);
    const redirectTo = from || ROLE_HOME_ROUTES[user.role] || "/";
  ...
  async function handleEmailLogin(event) {
    ...
    setSubmitting(true);
    try {
      const idToken = await loginWithEmail(email.trim(), password);
  ...
  return (
    <section className="mx-auto flex max-w-md flex-col px-4 py-12 sm:px-6">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Access your dashboard to report and track infrastructure issues.
      </p>
      ...
      <button
        ...
        disabled={submitting}
        className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-60"
      >
